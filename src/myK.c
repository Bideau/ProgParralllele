...
#include <math.h>
#include "struct.h"
int *k_means(Game *data, int n, int m, int k, double t, double **centroids){
...
do {
	...
	for (h = 0; h < n; h++) {
		...
		for (i = 0; i < k; i++) {
			...
			distance += pow(data[h].owners - c[i][0], 2);
			distance += pow(data[h].p2weeks - c[i][1], 2);
		...
		c1[labels[h]][0] += data[h].owners;
		c1[labels[h]][1] += data[h].p2weeks;
		counts[labels[h]]++;
